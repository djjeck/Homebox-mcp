...
const TEST_CASES: TestCase[] = [
  {
    ...
    async run({ callTool }) {
      ...
      if ((remainingLocations as any[]).length !== 0)
        throw new Error(
          `Expected 0 locations after wipe, got ${(remainingLocations as any[]).length}`
        );
...

  {
    ...
    async run({ callTool }) {
      const created = await callTool('create_location', {
        name: 'Test Location',
      ...
      if (fetched.name !== 'Test Location')
        throw new Error(`Expected name 'Test Location', got '${fetched.name}'`);
...

  {
    ...
    async run({ callTool }) {
      ...
      if (fetched.name !== 'test-tag')
        throw new Error(`Expected name 'test-tag', got '${fetched.name}'`);
...

  {
    ...
    async run({ callTool }) {
      ...
      if (fetched.name !== 'Test Item')
        throw new Error(`Expected 'Test Item', got '${fetched.name}'`);
      if (!Array.isArray(fetched.tags)) throw new Error('Expected item.tags to be an array');
      if (fetched.labels !== undefined)
        throw new Error('Expected item.labels to be absent (normalized to tags)');
      ...
      if (updated.name !== 'Test Item (updated)')
      ...
      if (partial.name !== 'Test Item (updated)')
        throw new Error(`Partial update wiped name, got '${partial.name}'`);
      ...
      if (typeof priced.purchasePrice !== 'number')
        throw new Error(
          `Expected purchasePrice to be a number, got ${typeof priced.purchasePrice}`
        );
      if (priced.purchasePrice !== 49.99)
        throw new Error(`Expected purchasePrice 49.99, got ${priced.purchasePrice}`);
      ...
      if (!Array.isArray(found.tags))
        throw new Error('Expected search result item.tags to be an array');
      if (found.labels !== undefined)
        throw new Error('Expected search result item.labels to be absent (normalized to tags)');
...

  {
    ...
    async run({ callTool }) {
      ...
      if (!Array.isArray(entries))
        throw new Error(`get_item_maintenance: expected array, got ${typeof entries}`);
      const found = entries.find((e: any) => e.id === entry.id);
      if (!found) throw new Error('get_item_maintenance: created entry not found');
      if (found.name !== 'Annual service')
        throw new Error(
          `get_item_maintenance: expected name 'Annual service', got '${found.name}'`
        );
      if (parseFloat(found.cost) !== 150)
        throw new Error(`get_item_maintenance: expected cost 150, got '${found.cost}'`);
      ...
      if (!Array.isArray(afterDelete))
        throw new Error('get_item_maintenance after delete: expected array');
  ...
  },
  {
    ...
    async run({ callTool }) {
      ...
      if (!Array.isArray(byLocA))
        throw new Error(`get_items_by_location: expected array, got ${typeof byLocA}`);
      if (!byLocA.find((i: any) => i.id === itemA.id))
        throw new Error('get_items_by_location: itemA missing from locA results');
      if (byLocA.find((i: any) => i.id === itemB.id))
        throw new Error('get_items_by_location: itemB leaked into locA results');

      // get_items_by_tag: tagA must return only itemA
      const byTagA = await callTool('get_items_by_tag', { tagId: tagA.id });
      if (!Array.isArray(byTagA))
        throw new Error(`get_items_by_tag: expected array, got ${typeof byTagA}`);
      if (!byTagA.find((i: any) => i.id === itemA.id))
        throw new Error('get_items_by_tag: itemA missing from tagA results');
      if (byTagA.find((i: any) => i.id === itemB.id))
        throw new Error('get_items_by_tag: itemB leaked into tagA results');
      ...
      if (!Array.isArray(searchByLocItems))
        throw new Error(`search_items+locationId: expected array, got ${typeof searchByLocItems}`);
      if (!searchByLocItems.find((i: any) => i.id === itemA.id))
        throw new Error('search_items+locationId: itemA missing');
      if (searchByLocItems.find((i: any) => i.id === itemB.id))
        throw new Error('search_items+locationId: itemB leaked in');
      ...
      if (!Array.isArray(searchByTagItems))
        throw new Error(`search_items+tagId: expected array, got ${typeof searchByTagItems}`);
      if (!searchByTagItems.find((i: any) => i.id === itemB.id))
        throw new Error('search_items+tagId: itemB missing');
      if (searchByTagItems.find((i: any) => i.id === itemA.id))
        throw new Error('search_items+tagId: itemA leaked in');

      // search_items with both locationId and tagId: only itemA (locA + tagA) should appear
      const searchBoth = await callTool('search_items', {
        query: 'Filter',
        locationId: locA.id,
        tagId: tagA.id,
      });
      const searchBothItems: any[] = searchBoth?.items ?? searchBoth ?? [];
      if (!Array.isArray(searchBothItems))
        throw new Error(`search_items+both: expected array, got ${typeof searchBothItems}`);
      if (!searchBothItems.find((i: any) => i.id === itemA.id))
        throw new Error('search_items+both: itemA missing');
      if (searchBothItems.find((i: any) => i.id === itemB.id))
        throw new Error('search_items+both: itemB leaked in');
  ...
  },
  {
    ...
    async run({ callTool, callToolRaw, readResource }) {
      ...
      if (!att)
        throw new Error(`Uploaded attachment '${ATTACHMENT_FILENAME}' not found in item response`);
      ...
      if (resourceLink.uri !== urlResult.url)
        throw new Error(
          `get_item_attachment: resource_link uri '${resourceLink.uri}' does not match url '${urlResult.url}'`
        );

      // 3. Content via HTTP URL
      const proxyResponse = await axios.get(urlResult.url, { responseType: 'text' });
      if (proxyResponse.status !== 200)
        throw new Error(`Proxy URL returned HTTP ${proxyResponse.status}`);
      if (proxyResponse.data !== ATTACHMENT_CONTENT)
        throw new Error(
          `HTTP content mismatch: expected ${JSON.stringify(ATTACHMENT_CONTENT)}, got ${JSON.stringify(proxyResponse.data)}`
        );
      ...
      if (resourceText !== ATTACHMENT_CONTENT)
        throw new Error(
          `resources/read content mismatch: expected ${JSON.stringify(ATTACHMENT_CONTENT)}, got ${JSON.stringify(resourceText)}`
        );
...

  {
    ...
    async run({ callTool }) {
      ...
      const locDefault = await callTool('create_location', {
        name: 'Collection Test Location (default)',
      });
      ...
      const otherLocsByName: any[] = await callTool('list_locations', {
        collection: otherCol.name,
      });
...

  {
    ...
    async run({ callTool }) {
      ...
      if (!Array.isArray(before))
        throw new Error('Expected array from list_locations before token invalidation');
      ...
      if (!Array.isArray(after))
        throw new Error('Expected array from list_locations after re-auth');
    },
  },
];
